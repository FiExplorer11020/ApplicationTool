...
## 10) Dépannage rapide (Windows / PowerShell)

### Erreur: `Super expression must either be null or a function`
- Cette erreur venait du rendu de `recharts` dans un composant serveur.
- Correctif appliqué: le graphique dashboard est déplacé dans un composant client (`components/dashboard/weekly-applications-chart.tsx`).

### Variables Supabase (important)
- Le projet lit `NEXT_PUBLIC_SUPABASE_ANON_KEY`.
- Certains écrans Supabase affichent `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_DEFAULT_KEY`.
- Si c'est ton cas, **mets la même valeur dans `NEXT_PUBLIC_SUPABASE_ANON_KEY`** dans `.env.local`.

### Si `npm install` ne marche pas dans PowerShell
```powershell
npm.cmd install
```
Ou:
